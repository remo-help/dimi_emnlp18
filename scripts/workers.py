...
def start_local_workers_with_distributer(work_distributer, maxLen, cpu_workers, gpu_workers=0,
                                         gpu=False, batch_size=1, K=None, D=None, random_gen=None):
    ...
    return start_local_workers(host=work_distributer.host, jobs_port=work_distributer.jobs_port,
                               ...
                               batch_size=batch_size, K=K, D=D, random_gen=random_gen)

def worker_run_proxy(worker_list, i, config_list=None, random_gen=None):
    worker = worker_list[i]
    if config_list is not None:
        config = config_list[i]
        host, jobs_port, results_port, models_port, maxLen, K, D, tid, gpu , batch_size, level, random_gen = config
        worker = PyzmqWorker(host, jobs_port, results_port, models_port, maxLen, K=K, D=D,
                                     tid=tid, gpu=gpu, batch_size=batch_size, level=level, random_gen=random_gen)
    worker.run()

def start_local_workers_old(host=None, jobs_port=None, results_port=None, models_port=None,
                        maxLen=None, K=None, D=None, cpu_workers=None,
                        gpu_workers=0, gpu=False, batch_size=1, random_gen=None):
    ...
    for i in range(0, cpu_workers+gpu_workers):
        ...
        worker_configs.append((host, jobs_port, results_port, models_port, maxLen, K, D,
                                     i, gpu, batch_size, logging.getLogger().getEffectiveLevel(), random_gen))
        workers.append(fs)
...
def start_local_workers(host=None, jobs_port=None, results_port=None, models_port=None,
                        ...
                        gpu_workers=0, gpu=False, batch_size=1, random_gen=None):
    logging.info(
        "Starting %d cpu workers and %d gpu workers at host %s with jobs_port=%d, results_port=%d, models_port=%d, maxLen=%d" % (
        cpu_workers, gpu_workers, host, jobs_port, results_port, models_port, maxLen))
    processes = []
    workers = []
    worker_configs = []
    for i in range(0, cpu_workers + gpu_workers):
